...
GGUF_FILE=`zenity --title='Select GGUF Model File' --text 'Select GGUF Model File' --file-selection --filename="$MODEL_DIR"  --file-filter='*.gguf'`
TEMP=`zenity --title='Select Temperature' --text 'Select Temperature (0 is more deterministic and less creative)' --entry --entry-text=0.0`
NGL=`zenity --title='Select Number of GPU Layers' --text 'Select Number of GPU Layers (0 if you have no GPU)' --scale --value=65 --min-value=0 --max-value=100`
